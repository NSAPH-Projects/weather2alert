...
class HeatAlertEnv(gym.Env):
    """A gym environment for optimizing heat alert issuance during heatwaves."""
    ...
    def __init__(
        ...
        obs_dim = self.baseline_dim + z + 3  # alert variables
    ...
    def reset(self, seed: int | None = None):
        ...
        self.allowed_alert_buffer = (
            []
        )  # what we allow the RL to do (based on the budget)
        self.t = 0  # day of summer indicator
        self.feature_ep_index = self.rng.choice(
            self.n_feature_episodes
        )  #  We call this a hybrid environment because it uses a model for the rewards but samples the real weather trajectories for each summer.
        ...
        if self.incorp_forecasts:
            if "N" in self.forecast_type:
                ...
                if self.forecast_error > 0:
                    err = np.random.uniform(
                        -self.forecast_error, self.forecast_error, 2
                    )
                    ef = ef * (1 + err)
            ...
            if "Av4" in self.forecast_type:
                ...
                if self.forecast_error > 0:
                    err = np.random.uniform(
                        -self.forecast_error, self.forecast_error, 4
                    )
                    ef = ef * (1 + err)
                extra_feats = extra_feats + ef
            if "Q" in self.forecast_type:
                ...
                if self.forecast_error > 0:
                    err = np.random.uniform(
                        -self.forecast_error, self.forecast_error, 6
                    )
                    ef = ef * (1 + err)
                extra_feats = extra_feats + ef
                    ...
                    if d < self.n_days:
                        if self.forecast_error == 0:
                            extra_feats = extra_feats + [
                                self.extra_states["future"][self.feature_ep_index, d]
                                - today
                            ]
                        ...
                        extra_feats = extra_feats + [
                            0
                        ]  # when it goes past the end of the summer
        total_prev_alerts = sum(self.allowed_alert_buffer)
        remaining_alerts = self.budget - total_prev_alerts
        prev_alerts_2wks = (
            sum(self.allowed_alert_buffer[-14:]) - self.prev_alert_mean
        ) / (2 * self.prev_alert_std)
        prev_alert_lag = (
            0 if len(self.allowed_alert_buffer) == 0 else self.allowed_alert_buffer[-1]
        )
        alert_feats = [remaining_alerts, prev_alerts_2wks, prev_alert_lag]

        return np.array(baseline_feats + extra_feats + alert_feats)
    ...
    def _get_reward(self, posterior_index, action, alert_feats):
        ...
        baseline = np.exp(
            sum(baseline_contribs)
            +
            # Note: remaining alerts is not a feature in the rewards model
            alert_feats[1]
            * self.posterior_coefficient_samples["baseline_previous_alerts"][
                posterior_index
            ]
            + alert_feats[2]
            * self.posterior_coefficient_samples["baseline_alert_lag1"][posterior_index]
            + self.posterior_coefficient_samples["baseline_bias"][posterior_index]
        )

        effectiveness = sigmoid(
            sum(effectiveness_contribs)
            +
            # Note: remaining alerts is not a feature in the rewards model
            alert_feats[1]
            * self.posterior_coefficient_samples["effectiveness_previous_alerts"][
                posterior_index
            ]
            + alert_feats[2]
            * self.posterior_coefficient_samples["effectiveness_alert_lag1"][
                posterior_index
            ]
        ...
        if self.penalize:
            if self.penalty_effect:
                ...
                r = (
                    1 - baseline - 10 * (self.penalty) ** ((self.t) / 25)
                )  # penalty = 0.1 seems good here
            else:
    ...
    def step(self, action: int):
        ...
        reward = np.mean(
            [
                self._get_reward(i, action, alert_feats=self.observation[-3:])
